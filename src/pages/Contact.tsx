...
import { getWhatsAppLink, CONTACT_EMAIL } from '../lib/contact';
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (publicKey && serviceId && templateId) {
        ...
        // 2. Fallback: open mailto: link so the user's email client sends it
      ...
      // 3. Always redirect to WhatsApp with all form details pre-filled
      const whatsappMsg =
      ...
      // Small delay so the user sees the success state before being taken to WhatsApp
      setTimeout(() => {
      ...
      // Even on error, still redirect to WhatsApp so the user can reach you
      const whatsappMsg =
